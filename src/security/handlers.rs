...
use crate::{
    ...
    middleware::CurrUser,
    security::schema::{
        NotifyParams, ResidentDetails, SecurityProfileDto, SecurityRegularDto, UpdatePfpParams,
        UpdateSecurityProfileSchema, VerifiedVisitorDetails, VerifiedVisitorParams, VisitorLogDto,
        VisitorSecurityDto,
    },
};
...
pub async fn get_security_by_email(
    ...
    match get_security_query.fetch_one(&data.db).await {
        Ok(security) => (StatusCode::OK, Json(security)).into_response(),
        Err(err) => {
            eprintln!("Error: {}", err);
...
pub async fn get_visitors(
    ...
    let security_society_query = query("SELECT society_id from users WHERE email = ?").bind(email);

    let security_society = match security_society_query.fetch_one(&data.db).await {
...
pub async fn verified_visitor_to_logs(
    ...
    let visitor_data = match get_visitor_data_query.fetch_one(&data.db).await {
        Ok(visitor_data) => visitor_data,
        Err(err) => {
            eprintln!("Could not fetch visitor data:{}", err);
            return GGError::DefunctCredentials(
                "Could not fetch visitor data with provided details",
            )
            .into_response();
        }
    };
    ...
    let remove_visitor_result = remove_from_visitors_query.execute(&data.db).await;
    ...
    match add_visitor_to_logs_query.execute(&data.db).await {
        Ok(_) => (
            StatusCode::OK,
            Json(json!({
...
pub async fn get_visitor_logs(
    ...
    let query_result = get_visitor_logs_query.fetch_all(&data.db).await;

    match query_result {
        Ok(visitor_logs) => (StatusCode::OK, Json(visitor_logs)).into_response(),
        Err(err) => {
...
pub async fn get_residents_to_notify(
    ...
    let resident_details_result = resident_details_query.fetch_all(&data.db).await;
