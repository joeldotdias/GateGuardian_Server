...
use crate::{
    ...
    middleware::CurrUser,
    resident::schema::{
        AddHomeDetailsSchema, AdminResidentDto, AdminSecurityDto, DashProfileDetails, NoticeDto,
        RegularDto, ResidentProfileDto, SaveNoticeSchema, SaveRegularSchema, SaveVisitorSchema,
        UpdatePfpParams, UpdateResidentProfileSchema, VisitorResidentDto,
    },
};

use super::schema::{SaveResidentParams, SocietyId};
...
pub async fn get_resident_by_email(
    ...
    let society_id_query = query("SELECT society_id FROM users WHERE email = ?").bind(&email);

    let society_id = match society_id_query.fetch_one(&data.db).await {
        Ok(resident) => resident.try_get::<i32, _>("society_id").unwrap(),
        Err(err) => {
    ...
    match get_resident_query.fetch_one(&data.db).await {
        Ok(resident) => (StatusCode::OK, Json(resident)).into_response(),
        Err(err) => {
            eprintln!("Error: {}", err);
...
pub async fn add_resident_home_details(
    ...
    let query_result = update_details_query.execute(&data.db).await;
...
pub async fn update_resident_profile(
    ...
    let query_result = update_profile_query.execute(&data.db).await;
...
pub async fn update_resident_pfp(
    ...
    let query_result = update_pfp_query.execute(&data.db).await;
...
pub async fn get_visitors(
    ...
    let resident_id_query = query("SELECT resident_id FROM residents WHERE email = ?").bind(email);

    let resident_id_query_result = resident_id_query.fetch_one(&data.db).await;

    let resident_id = match resident_id_query_result {
        Ok(resident) => resident.try_get::<i32, _>("resident_id").unwrap(),
        Err(err) => {
    ...
    let query_result = get_visitors_query.fetch_all(&data.db).await;

    match query_result {
        Ok(visitors) => (StatusCode::OK, Json(visitors)).into_response(),
        Err(err) => {
...
pub async fn save_visitor(
    ...
    match save_visitor_query.execute(&data.db).await {
        Ok(_) => (
            StatusCode::OK,
            Json(json!({
...
pub async fn get_recent_visitor_otp(
    ...
    let get_code_query_result = get_code_query.fetch_one(&data.db).await;
...
pub async fn get_regulars(
    ...
    let regulars_query_result = get_regulars_query.fetch_all(&data.db).await;

    match regulars_query_result {
        Ok(regulars) => (StatusCode::OK, Json(regulars)).into_response(),
        Err(err) => {
...
pub async fn save_regular(
    ...
    let society_id_query_result = society_id_query.fetch_one(&data.db).await;
    ...
    match save_regular_query.execute(&data.db).await {
        Ok(_) => (
            StatusCode::OK,
            Json(json!({
...
pub async fn get_residents_by_society(
    ...
    let get_residents_query_result = get_residents_query.fetch_all(&data.db).await;
...
pub async fn get_security_by_society(
    ...
    let get_securities_query_result = get_securities_query.fetch_all(&data.db).await;

    match get_securities_query_result {
        Ok(securities) => (StatusCode::OK, Json(securities)).into_response(),
        Err(err) => {
...
pub async fn save_resident(
    State(data): State<Arc<AppState>>,
    Extension(curr_user): Extension<CurrUser>,
    Query(params): Query<SaveResidentParams>,
) -> impl IntoResponse {
    let admin_email = curr_user.email;
    println!("Got {}", admin_email);

    let result = query_as::<_, SocietyId>(
        "SELECT society_id FROM users WHERE email = ? AND category = 'admin'",
    )
    .bind(&admin_email)
    .fetch_one(&data.db)
    .await;

    let society_id = match result {
        Ok(SocietyId {
            society_id: Some(id),
        }) => id,
        _ => return GGError::Stupidity("Could not fetch security details").into_response(),
    };

    println!("Id {}", society_id);

    let insert_user_result = query(
        "
        INSERT INTO users (name, email, category, society_id)
        VALUES (?, ?, 'resident', ?)
        ",
    )
    .bind(&params.name)
    ...
    let insert_resident_result = query(
        "
        INSERT INTO residents (email)
        VALUES (?)
        ",
    )
    .bind(&params.email)
    ...
    match insert_resident_result {
        Ok(_) => StatusCode::OK.into_response(),
        Err(_) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            "Failed to insert into residents",
        )
            .into_response(),
    }
}
...
pub async fn get_notices(
    ...
    match get_notices_query.fetch_all(&data.db).await {
...
pub async fn add_notice(
    ...
    let society_id = match get_society_query {
        ...
        Err(err) => {
            eprintln!("Could not find society: {}", err);
            return GGError::DefunctCredentials("Could not find your society details")
                .into_response();
        }
    };
    ...
    match add_notice_query.execute(&data.db).await {
        Ok(_) => (
            StatusCode::CREATED,
            Json(json!({
                "msg": "Notice added"
